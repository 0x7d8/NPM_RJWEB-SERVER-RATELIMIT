...
const checkRule = (rule, ctr) => {
  if (rule.ignore)
    if (Array.isArray(rule.ignore)) {
      ...
      rule.ignore.forEach((ignore) => {
        if (doContinue)
          return;
        if ((0, import_types.isRegExp)(ignore))
          doContinue = !ignore.test(ctr.url.path);
        else
          doContinue = !ctr.url.path.includes((0, import_rjweb_server.pathParser)(ignore));
      });
      if (!doContinue)
        return false;
    } else {
      let doContinue = true;
      if ((0, import_types.isRegExp)(rule.ignore))
        doContinue = !rule.ignore.test(ctr.url.path);
      else
        doContinue = !ctr.url.path.includes((0, import_rjweb_server.pathParser)(rule.ignore));
      if (!doContinue)
        return false;
    }
  if (Array.isArray(rule.path)) {
    ...
    rule.path.forEach((path) => {
      if ((0, import_types.isRegExp)(path))
        returns = path.test(ctr.url.path);
      else
        returns = ctr.url.path.includes((0, import_rjweb_server.pathParser)(path));
    });
    return returns;
  } else {
    if ((0, import_types.isRegExp)(rule.path))
      return rule.path.test(ctr.url.path);
    else
      return ctr.url.path.includes((0, import_rjweb_server.pathParser)(rule.path));
  }
};
